...
- **2.2 — Base URL / Gateway.** Point the listing's base URL at JK's **direct Cloud Function URL** (avoids the Firebase Hosting 60s edge timeout that bit JK — `/recommend` runs 60–180s). Confirm RapidAPI's own proxy timeout accommodates this; if not, the long endpoints route through the async `/v1/jobs` pattern instead (note as fallback, not v1 default).
...
1. **Long-endpoint timeout through RapidAPI's proxy.** `/recommend` can run 60–180s. If RapidAPI's gateway caps below that, switch those endpoints to the async `/v1/jobs` pattern for the RapidAPI surface (more work — defer unless 2.2 testing shows a hard cap). **Resolve during Phase 2.2.**
