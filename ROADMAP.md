...
- ◐ CONN-1 PLAN LOCKED 2026-06-07 (8-task session list): **Phase 1** = fix the recommend contamination bug ([[project_recommend_contamination]], un-parked — submission blocker) + **async job pattern** for `recommend`/`recommend_deep` (60–180s latency exceeds Firebase Hosting's 60s edge timeout on the `/api/**` rewrite that the `jackpotkeywords.web.app/api/mcp` listing URL depends on → tools start a Firestore-backed job and return a job id; new `jackpotkeywords_get_report` tool polls/fetches; meter at job start, refund on failure). **Phase 2** = full 6-tool set on the remote endpoint (currently only `recommend`+`usage_status`; add `recommend_deep`/`audit`/`aeo_scan`/`credit_balance` w/ annotations) + e2e re-verify in Claude on the web.app URL. **Phase 3** = assets (favicon on web.app EARLY — Google s2 cache lags days; square logo; 3–5 screenshots ≥1000px; public docs page w/ troubleshooting; reviewer account `mcp-review@anthropic.com` + create-if-missing credit grant; AuthKit email self-signup ON). **Phase 4** = submit per playbook §2 (category: Business & Productivity or Data & Analytics). No custom domain needed — ships on `jackpotkeywords.web.app`.
- ☐ ANNOT-1: Add MCP tool annotations to all 5 tools — `readOnlyHint: true` on the read/scan/recommend tools (`aeo_scan`, `audit`, `recommend`, `recommend_deep`, `credit_balance`); none are destructive. **Prereq for CONN-1** — missing annotations are the #1 Claude Connector Directory rejection cause (~30% of rejections). Cheap, no behavior change. Mirror into the npm/Smithery/Glama manifests.
- ◐ CONN-1: Submit to the **Claude Connector Directory** (`claude.com/docs/connectors/building/submission`, ~2wk review). **INFRA DONE 2026-06-03 (commit 1cce9ae): remote MCP endpoint live at `/api/api/mcp` with WorkOS AuthKit OAuth 2.1 + RFC 9728 PRM + tool annotations + privacy policy (`/privacy`) — verified live.** Remaining (USER): Dev-Mode-test the connector in Claude (add the MCP URL, complete WorkOS login, confirm tools run), then submit. Full plan + next steps: `docs/api-deployment/CONNECTOR-DIRECTORIES-PLAN-2026-06-03.md` + [[project_connector_directories]]. Needs a remote MCP endpoint (Streamable HTTP, not the stdio npm server) + OAuth 2.1, privacy-policy URL, tool annotations (ANNOT-1), a test account, production-ready status — all now met. **Reuses the exact remote-MCP + OAuth 2.1 lift already scoped in SURF-3 (OpenAI Apps SDK)** — build the remote transport once, submit to both Anthropic + OpenAI directories. Highest-leverage discovery surface (one click inside Claude).
