...
- ☐ TREND-6 (blocked): **Official Google Trends API (alpha)** — apply for access now; treat as a later upgrade path over the BigQuery approach (TREND-2). Invite-gated, no dependency until granted.

### Discovery tools — two NEW products (2026-06-14)
*Strategic pivot from "I have a product → keywords" (pull) to "I don't know what to do → here's what's rising" (push) — a broader, more viral top-of-funnel for JK's awareness problem ([[project_demand_diagnosis]]). Two distinct tools, deliberately split. **Sequencing:** the trend-data integration above (TREND-1..6) lands FIRST (it's the shared data spine); these features come after. **ToS RESOLVED 2026-06-14** (doc: `docs/api-deployment/DISCOVERY-SOURCES-TOS-STRATEGY-2026-06-14.md`): extend the composite-score defense ([[project_ads_tos_derived_score]]) — non-invertible scores, free/open anchors (Wikipedia/GDELT) load-bearing, restricted Google sources (KP/Trends/YouTube) ≤~25-30% minority; **no banding** (show raw at same fidelity as Keyword Search); **scrape/paid sources ABANDONED** (Reddit-commercial, marketplace, 1688 → no profitability/sourcing features); consumer-feature-first (attorney opinion only if metered public API). Interactive mockups built 2026-06-14. **Build plans (2026-06-14):** `docs/discovery/PRODUCT-SEARCH-PLAN-2026-06-14.md` (Phase 0 spine → niche dataset/Goldmine → board → Option B report → billing; BUILD FIRST) + `docs/discovery/MEDIA-TOPIC-SEARCH-PLAN-2026-06-14.md` (topic spine → Interest Score → two-mode UI → retention; SECOND).*

- ☐ DISC-1: **Products tool — "Product Search."** New tab: a standing, daily-refreshed board of *rising sellable niches* with proven demand. Columns: niche, momentum (sparkline + %), volume, CPC, competition, **Goldmine Score** (commerce-weighted), **Stage** (Emerging/Rising/Peaking/Cooling — the timing differentiator), category, **Sell-channel coverage badges** (Amazon/Etsy/YT/Pin/eBay/TikTok — *coverage from search-suggest, NOT per-platform momentum*; ★ = Gemini best-fit channel), and "Research →" handoff into the existing `recommend` pipeline. Free tier = top ~5 rows with volume/CPC blurred + paywall; Pro = full board + filters + CSV export. **Data:** Trends BigQuery + KP volume/CPC + Gemini clustering + the free suggest endpoints we already pull. **Mockup: `product-search-mockup.html`** (interactive: Free↔Pro toggle, sort, stage chips, platform filter). Atomic row = a niche. (Name: "Product Search" — distinct from existing "Keyword Search" tab; watch for confusion.)
- ☐ DISC-2: **Social/Topics tool — "Media Topic Search"** — for creators/influencers ("what should I make next"). PURELY a demand/interest tool — NO commerce/CPC scoring. Surfaces *what people are searching/reading about per platform* (query-demand, not "what's trending on TikTok" — that's honestly unsourceable for free). **Two view modes (both wanted):** (a) pick a platform → see its rising query themes; (b) pick a topic → see how it's searched across all platforms. **Momentum (decided 2026-06-14):** topic-level momentum is REAL (Wikipedia Pageviews + Google Trends time-series); **per-platform momentum is a ≈PROXY** (re-polled search-suggest breadth, only accrues over time, clearly labeled "proxy" in UI) — added LATER, not day one. Never imply a platform's internal trend feed. **Free-spine data (none used today):** Wikipedia Pageviews API (no-auth, best interest-over-time signal), GDELT 2.0 (free/open news themes, BigQuery), YouTube Data API v3 (`videos.mostPopular`, 10k units/day free), Hacker News API (tech), RSS feeds (entertainment/news incl. WorldStarHipHop — no official API, RSS only), + Etsy/TikTok suggest endpoints (new). **Gated/deferred:** Reddit (free signal but commercial use needs contract — ToS item), Pinterest Trends (sandbox-capped), TikTok Creative Center (web-only=scrape), X/Twitter ($200/mo, skip). **Mockup: `media-topic-search-mockup.html`** (interactive: By-platform ↔ By-topic mode toggle, violet Interest Score to distinguish from gold commerce, ≈proxy-tagged per-platform momentum, Explore→ jumps a row into the cross-platform topic view, Free↔Pro). Atomic row = a topic OR a platform (per mode).
- ☐ DISC-3: **Shared scoring decision** — Product Search uses the commerce-weighted Goldmine Score; Media Topic Search needs a *separate* interest/demand score (CPC meaningless for video topics). Don't reuse one score across both. Ties to [[project_demand_score]].
- ☐ DISC-4: **"Research →" handoff — Niche Opportunity Report (Option B).** Clicking Research on a Product Search niche opens a FREE intermediate report (revives the planned `ConceptReport`, roadmap P3-2, at niche scope); the paid action is the "Run full keyword research" CTA → existing `recommend` pipeline (Option A). **Built ONLY from data we can source (research 2026-06-14):** demand/volume + 90d/5yr trend + seasonality (KP + Trends), Opportunity Score breakdown ([[project_demand_score]]), CPC range + **advertising** competition (KP — explicitly NOT "SEO difficulty", which needs paid backlink data), Google Ads budget-to-clicks forecast, related sub-niches + competitor brand discovery (Gemini — names only, no market share), sell-channel fit badges, 12-of-1000 keyword preview. **PERMANENTLY EXCLUDED (decision 2026-06-14):** profit margin, units sold, supplier cost, marketplace saturation — all required scraping (Amazon/Etsy) or paid-data resale (1688/Alibaba), which we've **abandoned**. No "Sourcing module." The report instead shows an honest **"what we don't do (and why)"** trust note. Honesty rules: (1) "advertising competition" not "SEO difficulty"; (2) competitor names, never fabricated market share. **Mockup: `niche-opportunity-report-mockup.html`** (demo switches Emerging/Rising/Cooling niches so verdict + score change). Decided: ship **B as a FREE pitch → A (`recommend`) as paid depth.**
...
- ✓ 2026-06-12 — **WEBMCP-ATTR: WebMCP usage + revenue wired into the seller-dashboard MCP/API view. DEPLOYED to prod (even-plate-378520).** WebMCP runs on the CONSUMER credit pool (Firebase session), so the API-customer rollups in `admin.ts` never saw it. Wiring:
  - **Usage:** `'webmcp'` added to `ApiSource`/`API_SOURCES` (`services/apiCredits.ts`); new `services/webmcpRollup.ts` (`recordWebmcpUsage(userId, email)` — atomic increment on `usage_rollups/webmcp`, excludes admin/test at write-time); called after each successful op in `api/webmcp.ts`; `admin.ts` merges the counter into `usageBySource.webmcp`.
  - **Revenue (first-touch):** `recordWebmcpUsage` also stamps `users/{uid}.attributionSource='webmcp'` (first-touch-wins). Stripe webhook (`services/stripe.ts`) records `amountCents` + the user's `attributionSource` on the purchase AND mirrors tagged top-ups into a dedicated top-level `consumerAttributedPurchases` collection (auto-indexed — avoids the disabled collection-group index on per-user `transactions`). `admin.ts` reads that collection and buckets only explicitly-tagged surfaces into `revenueBySource.webmcp` — untagged website top-ups stay excluded (same rule as the API side).
  - **Signups stay 0 by design** — WebMCP isn't an acquisition channel (you must already be signed in to reach it).
  - Verified live: deployed `admin/stats` returns `usageBySource.webmcp` + `revenueBySource.webmcp` buckets (0 until real traffic/top-ups). Dashboard surface added (sellerdashboard `platforms.json` → JK `traffic_surfaces`). tsc + build clean.
  - ATTRIBUTION CAVEAT: first-touch among attributable surfaces — a website-origin user who later uses WebMCP gets tagged 'webmcp' (no prior tag exists to preserve); measures "revenue from users who engage via WebMCP," not "acquired via WebMCP."
